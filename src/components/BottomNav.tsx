...
const BottomNav: React.FC<BottomNavProps> = ({ appState, onNewConsultation, onViewHistory, hasDiagnosisResults, onViewResults }) => {
    ...
    return (
        <nav className="sticky bottom-0 left-0 right-0 bg-white dark:bg-slate-800 shadow-[0_-2px_5px_rgba(0,0,0,0.1)] md:hidden">
            <div className="flex justify-around">
                <button 
                    onClick={onViewResults} 
                    disabled={!hasDiagnosisResults}
                    className={getButtonClass(appState === AppState.Diagnosis)}
