...
const Logo: React.FC<LogoProps> = ({ className = 'h-10 w-auto', variant = 'default' }) => {
    const isInverse = variant === 'inverse';

    const textColor = isInverse ? '#FFFFFF' : '#0f172a';
    const iconFill = isInverse ? '#FFFFFF' : 'url(#logoGradient)';
    const crossStroke = isInverse ? '#156dbd' : '#FFFFFF';
